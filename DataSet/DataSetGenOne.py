...
def main():
    ...
    output_folder = "output_imag/65"
    temp_folder = "images"

    # Создаем выходную директорию, если она не существует
    os.makedirs(output_folder, exist_ok=True)

    # Определяем начальный индекс изображений
    max_image_index = get_max_image_index(output_folder)

    for i in range(1, 71):  # Обрабатываем изображения от 1.png до 70.png
        ...
        # Переносим все файлы из temp_folder напрямую в output_folder
        for file_name in os.listdir(temp_folder):
            src_path = os.path.join(temp_folder, file_name)

            # Присваиваем новый уникальный индекс
            max_image_index += 1
        ...
        print(f"Файлы из {temp_folder} перемещены в {output_folder}.")

        # Очищаем папку temp_folder
